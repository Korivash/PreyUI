...
-- Applies PREY color scheme with dynamic content-height backdrop
---------------------------------------------------------------------------
...
-- Style completion checkmark with PREY color
...
-- Sync PREY max height with Blizzard's editModeHeight so truncation matches backdrop
...
local function ApplyPREYBackdrop(trackerFrame, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    -- Create PREY backdrop (anchors will be set by UpdateBackdropAnchors)
...
local function SkinObjectiveTracker()
    ...
    -- Apply PREY backdrop with our colors/opacity
