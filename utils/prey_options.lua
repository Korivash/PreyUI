...
local function RefreshAll()
    if PREYCore and PREYCore.RefreshAll then PREYCore:RefreshAll() end
end

local function RefreshMinimap()
    if PREYCore and PREYCore.Minimap and PREYCore.Minimap.Refresh then PREYCore.Minimap:Refresh() end
end

local function RefreshUIHider()
    if _G.PreyUI_RefreshUIHider then _G.PreyUI_RefreshUIHider() end
end
...
local function RefreshUnitFrames(unit)
    if PREYCore and PREYCore.UnitFrames then
        ...
        if type(unit) == "string" then
            ...
            PREYCore.UnitFrames:RefreshFrames()
        end
    end
end
...
local function CreateAppearancePage(parent)
    ...
    GUI:SetSearchContext({tabIndex = 1, tabName = "Appearance & Theme"})

    -- Shared: push all skinned frames live after any color change
    local function RefreshAllSkinning()
        if _G.PreyUI_RefreshKeystoneColors        then _G.PreyUI_RefreshKeystoneColors()        end
        if _G.PreyUI_RefreshAlertColors           then _G.PreyUI_RefreshAlertColors()           end
        if _G.PreyUI_RefreshLootColors            then _G.PreyUI_RefreshLootColors()            end
        if _G.PreyUI_RefreshMPlusTimerColors      then _G.PreyUI_RefreshMPlusTimerColors()      end
        if _G.PreyUI_RefreshCharacterFrameColors  then _G.PreyUI_RefreshCharacterFrameColors()  end
        if _G.PreyUI_RefreshInspectColors         then _G.PreyUI_RefreshInspectColors()         end
        if _G.PreyUI_RefreshPowerBarAltColors     then _G.PreyUI_RefreshPowerBarAltColors()     end
        if _G.PreyUI_RefreshGameMenuColors        then _G.PreyUI_RefreshGameMenuColors()        end
        if _G.PreyUI_RefreshOverrideActionBarColors  then _G.PreyUI_RefreshOverrideActionBarColors()  end
        if _G.PreyUI_RefreshObjectiveTrackerColors   then _G.PreyUI_RefreshObjectiveTrackerColors()   end
        if _G.PreyUI_RefreshInstanceFramesColors  then _G.PreyUI_RefreshInstanceFramesColors()  end
        if _G.PreyUI_RefreshReadyCheckColors      then _G.PreyUI_RefreshReadyCheckColors()      end
    end

    if not (db and db.general) then
    ...
    ---------------------------------------------------------------------------
    GUI:SetSearchSection("Accent Color")

    local accentHeader = GUI:CreateSectionHeader(content, "Accent Color")
    accentHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - accentHeader.gap

    local accentDesc = GUI:CreateLabel(content,
        "Controls border highlights across all skinned frames — alerts, keystone, loot, game menu, trackers, and more.",
        11, C.textMuted)
    accentDesc:SetPoint("TOPLEFT", PAD, y)
    accentDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    accentDesc:SetJustifyH("LEFT")
    accentDesc:SetWordWrap(true)
    accentDesc:SetHeight(28)
    y = y - 34

    local customColorPicker  -- forward declare for enable/disable coupling

    local classColorCheck = GUI:CreateFormCheckbox(content, "Use My Class Color", "skinUseClassColor", general, function()
    ...
    customColorPicker = GUI:CreateFormColorPicker(content, "Custom Accent Color", "skinCustomColor", general, RefreshAllSkinning, { noAlpha = true })
    customColorPicker:SetPoint("TOPLEFT", PAD, y)
    customColorPicker:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    customColorPicker:SetEnabled(not general.skinUseClassColor)
    y = y - FORM_ROW

    ---------------------------------------------------------------------------
    -- SECTION: Background Darkness
    ---------------------------------------------------------------------------
    GUI:SetSearchSection("Background Darkness")

    y = y - 6

    local bgHeader = GUI:CreateSectionHeader(content, "Background Darkness")
    bgHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - bgHeader.gap

    local bgDesc = GUI:CreateLabel(content,
        "Sets how dark the backdrop is on all PreyUI skinned frames. Use the presets for quick results, or fine-tune with the color picker.",
        11, C.textMuted)
    bgDesc:SetPoint("TOPLEFT", PAD, y)
    bgDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    bgDesc:SetJustifyH("LEFT")
    bgDesc:SetWordWrap(true)
    bgDesc:SetHeight(28)
    y = y - 34

    -- Preset definitions: {label, bg_rgba}
    local PRESETS = {
        { label = "Dark",       color = {0.05, 0.05, 0.05, 0.90}, desc = "Soft dark" },
        { label = "Darker",     color = {0.02, 0.02, 0.02, 0.95}, desc = "Deep black" },
        { label = "Pure Black", color = {0.00, 0.00, 0.00, 1.00}, desc = "Opaque" },
    }

    -- Preset label row
    local presetLabel = content:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    presetLabel:SetFont(GUI.FONT_PATH, 12, "")
    presetLabel:SetTextColor(C.text[1], C.text[2], C.text[3], 1)
    presetLabel:SetText("Quick Presets")
    presetLabel:SetPoint("TOPLEFT", PAD, y)
    y = y - 24

    -- Row of 3 preset buttons
    local PRESET_BTN_HEIGHT = 50
    local presetRow = CreateFrame("Frame", nil, content)
    presetRow:SetPoint("TOPLEFT", PAD, y)
    presetRow:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    presetRow:SetHeight(PRESET_BTN_HEIGHT)

    local numPresets = #PRESETS
    local btnRefs = {}

    for i, preset in ipairs(PRESETS) do
        local btn = CreateFrame("Button", nil, presetRow, "BackdropTemplate")
        ...
        local pc = preset.color
        btn:SetBackdropColor(pc[1], pc[2], pc[3], pc[4])
        btn:SetBackdropBorderColor(C.border[1], C.border[2], C.border[3], 1)

        -- Initial anchor placeholders — OnSizeChanged does the real layout
        btn:SetPoint("TOP",    presetRow, "TOP")
        btn:SetPoint("BOTTOM", presetRow, "BOTTOM")

        btnRefs[i] = { btn = btn, preset = preset }

        -- Label inside
        local nameText = btn:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        nameText:SetFont(GUI.FONT_PATH, 12, "OUTLINE")
        nameText:SetTextColor(1, 1, 1, 1)
        nameText:SetText(preset.label)
        nameText:SetPoint("CENTER", btn, "CENTER", 0, 8)

        local descText = btn:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        descText:SetFont(GUI.FONT_PATH, 10, "")
        descText:SetTextColor(0.75, 0.75, 0.75, 1)
        descText:SetText(preset.desc)
        descText:SetPoint("CENTER", btn, "CENTER", 0, -8)

        btn:SetScript("OnClick", function()
            general.skinBgColor = { pc[1], pc[2], pc[3], pc[4] }
            -- Sync the color picker swatch if it exists
            if btn._bgColorPicker then
                btn._bgColorPicker.SetColor(pc[1], pc[2], pc[3], pc[4])
            end
            -- Highlight selected preset, dim others
            for _, ref in ipairs(btnRefs) do
                local isActive = (ref.preset == preset)
                pcall(ref.btn.SetBackdropBorderColor, ref.btn,
                    isActive and C.accent[1]  or C.border[1],
                    isActive and C.accent[2]  or C.border[2],
                    isActive and C.accent[3]  or C.border[3],
                    1)
            end
            RefreshAllSkinning()
        end)
        ...
        btn:SetScript("OnEnter", function(self)
            pcall(self.SetBackdropBorderColor, self,
                C.accentLight[1], C.accentLight[2], C.accentLight[3], 1)
        end)
        btn:SetScript("OnLeave", function(self)
            -- Re-apply correct border (active = accent, inactive = border)
            local isActive = (general.skinBgColor and
                general.skinBgColor[1] == pc[1] and
                general.skinBgColor[2] == pc[2] and
                general.skinBgColor[3] == pc[3])
            pcall(self.SetBackdropBorderColor, self,
                isActive and C.accent[1]  or C.border[1],
                isActive and C.accent[2]  or C.border[2],
                isActive and C.accent[3]  or C.border[3],
    ...
    -- Use OnSizeChanged on presetRow to lay out buttons responsively
    presetRow:SetScript("OnSizeChanged", function(self, width)
        local totalGap = 6 * (numPresets - 1)
        local btnW = math.floor((width - totalGap) / numPresets)
        for i, ref in ipairs(btnRefs) do
            ref.btn:ClearAllPoints()
            ref.btn:SetWidth(btnW)
            ref.btn:SetPoint("TOP",    self, "TOP")
            ref.btn:SetPoint("BOTTOM", self, "BOTTOM")
            ref.btn:SetPoint("LEFT",   self, "LEFT", (i - 1) * (btnW + 6), 0)
        end
    end)

    -- Trigger initial layout after registration
    C_Timer.After(0, function()
        if presetRow:GetWidth() > 0 then
            presetRow:GetScript("OnSizeChanged")(presetRow, presetRow:GetWidth())
        end
    end)

    y = y - PRESET_BTN_HEIGHT - 10

    -- Fine-tune color picker
    local bgColorPicker = GUI:CreateFormColorPicker(content, "Background Color (Fine Tune)", "skinBgColor", general, RefreshAllSkinning, { hasAlpha = true })
    bgColorPicker:SetPoint("TOPLEFT", PAD, y)
    bgColorPicker:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Give each preset button a reference so clicking sync the picker swatch
    for _, ref in ipairs(btnRefs) do
        ref.btn._bgColorPicker = bgColorPicker
    end

    -- Highlight whichever preset matches the current saved bg color on open
    C_Timer.After(0.05, function()
        local saved = general.skinBgColor
        if not saved then return end
        for _, ref in ipairs(btnRefs) do
            local pc = ref.preset.color
            local isActive = (math.abs(saved[1] - pc[1]) < 0.01 and
                              math.abs(saved[2] - pc[2]) < 0.01 and
                              math.abs(saved[3] - pc[3]) < 0.01)
            pcall(ref.btn.SetBackdropBorderColor, ref.btn,
                isActive and C.accent[1] or C.border[1],
                isActive and C.accent[2] or C.border[2],
                isActive and C.accent[3] or C.border[3],
    ...
    local scaleHeader = GUI:CreateSectionHeader(content, "UI Scale")
    scaleHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - scaleHeader.gap

    local scaleDesc = GUI:CreateLabel(content,
        "Scales the entire WoW interface. 1.0 is Blizzard default. Reload is required for the change to apply.",
        11, C.textMuted)
    scaleDesc:SetPoint("TOPLEFT", PAD, y)
    scaleDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    scaleDesc:SetJustifyH("LEFT")
    scaleDesc:SetWordWrap(true)
    scaleDesc:SetHeight(28)
    y = y - 34

    -- Initialise scale default if missing
    if db.general.uiScale == nil then
        db.general.uiScale = math.floor(UIParent:GetScale() * 100 + 0.5) / 100
    end

    local scaleSlider = GUI:CreateFormSlider(content, "UI Scale", 0.64, 1.20, 0.01,
        "uiScale", db.general, function(val)
            UIParent:SetScale(val)
            -- Round-trip save
            db.general.uiScale = math.floor(val * 100 + 0.5) / 100
        end)
...
local function CreateGeneralQoLPage(parent)
    ...
    local function RefreshAll()
        ...
        if PREYCore then
            ...
            if PREYCore.UpdateSecondaryPowerBar then
        ...
        C_Timer.After(0.1, function()
            if PREYCore and PREYCore.ApplyViewerLayout then
                ...
                PREYCore:ApplyViewerLayout("UtilityCooldownViewer")
            end
        end)
    end
    ...
    local function BuildGeneralTab(tabContent)
        ...
        if db and db.general then
            local scaleSlider = GUI:CreateFormSlider(tabContent, "Global UI Scale", 0.3, 2.0, 0.01,
                "uiScale", db.general, function(val)
                    ...
                    if PREYCore and PREYCore.UIMult then PREYCore:UIMult() end
            ...
            local function ApplyPreset(val, name)
                ...
                pcall(function() UIParent:SetScale(val) end)
                local msg = "|cffef4444[PreyUI]|r UI scale set to " .. val
        ...
        if combatTextDB then
            local combatTextCheck = GUI:CreateFormCheckbox(tabContent, "Enable Combat Text", "enabled", combatTextDB, function(val)
                if _G.PreyUI_RefreshCombatText then _G.PreyUI_RefreshCombatText() end
            end)
        ...
        if combatTimerDB then
            ...
            local previewCheck = GUI:CreateFormCheckbox(tabContent, "Preview Combat Timer", "enabled", previewState, function(val)
                if _G.PreyUI_ToggleCombatTimerPreview then
                    _G.PreyUI_ToggleCombatTimerPreview(val)
                end
            end)
        ...
        previewBtn:SetScript("OnClick", function()
            if _G.PreyUI_ToggleRaidBuffsPreview then
                local isPreview = _G.PreyUI_ToggleRaidBuffsPreview()
        ...
        local qsEnableCheck = GUI:CreateFormCheckbox(tabContent, "Enable Quick Salvage", "enabled", qsDB, function()
            if _G.PreyUI_RefreshQuickSalvage then _G.PreyUI_RefreshQuickSalvage() end
        end)
        ...
        local fontSizeSlider = GUI:CreateFormSlider(tabContent, "Key Tracker Font Size", 7, 12, 1, "keyTrackerFontSize", db.general, function()
            if _G.PreyUI_RefreshKeyTrackerFonts then
                _G.PreyUI_RefreshKeyTrackerFonts()
            end
        end)
        fontSizeSlider:SetPoint("TOPLEFT", PADDING, y)
        fontSizeSlider:SetPoint("RIGHT", tabContent, "RIGHT", -PADDING, 0)
        ...
        if minimapBtnDB then
            local showMinimapIconCheck = GUI:CreateFormCheckbox(tabContent, "Hide PREY Minimap Icon", "hide", minimapBtnDB, function(dbVal)
                local LibDBIcon = LibStub("LibDBIcon-1.0", true)
                if LibDBIcon then
                    if dbVal then
                        LibDBIcon:Hide("PreyUI")
                    else
                        LibDBIcon:Show("PreyUI")
                    end
                end
            end)
        ...
        local panelAlphaSlider = GUI:CreateFormSlider(tabContent, "PREY Panel Transparency", 0.3, 1.0, 0.01, "configPanelAlpha", db, function(val)
    ...
    local function BuildHUDVisibilityTab(tabContent)
        ...
        local cdmMouseoverCheck = GUI:CreateFormCheckbox(tabContent, "Show On Mouseover", "showOnMouseover", cdmVis, function()
            RefreshCDMVisibility()
            if _G.PreyUI_RefreshCDMMouseover then
                _G.PreyUI_RefreshCDMMouseover()
            end
        end)
        ...
        local function RefreshUnitframesVisibility()
            if _G.PreyUI_RefreshUnitframesVisibility then
                _G.PreyUI_RefreshUnitframesVisibility()
            end
        end
        ...
        local ufMouseoverCheck = GUI:CreateFormCheckbox(tabContent, "Show On Mouseover", "showOnMouseover", ufVis, function()
            RefreshUnitframesVisibility()
            if _G.PreyUI_RefreshUnitframesMouseover then
                _G.PreyUI_RefreshUnitframesMouseover()
            end
        end)
        ...
        local function RefreshCustomTrackersVisibility()
            if _G.PreyUI_RefreshCustomTrackersVisibility then
                _G.PreyUI_RefreshCustomTrackersVisibility()
            end
        end
        ...
        local ctMouseoverCheck = GUI:CreateFormCheckbox(tabContent, "Show On Mouseover", "showOnMouseover", ctVis, function()
            RefreshCustomTrackersVisibility()
            if _G.PreyUI_RefreshCustomTrackersMouseover then
                _G.PreyUI_RefreshCustomTrackersMouseover()
            end
        end)
    ...
    local function BuildChatTab(tabContent)
        ...
        if db and db.chat then
            ...
            local enableCheck = GUI:CreateFormCheckbox(tabContent, "PREY Chat Module", "enabled", chat, RefreshChat)
            enableCheck:SetPoint("TOPLEFT", PADDING, y)
            enableCheck:SetPoint("RIGHT", tabContent, "RIGHT", -PADDING, 0)
            y = y - FORM_ROW
            ...
            local introInfo = GUI:CreateLabel(tabContent, "Display the PREY reminder tips when you log in.", 10, C.textMuted)
    ...
    local function BuildTooltipTab(tabContent)
        ...
        local enableCheck = GUI:CreateFormCheckbox(tabContent, "PREY Tooltip Module", "enabled", tooltip, RefreshTooltips)
        enableCheck:SetPoint("TOPLEFT", PADDING, y)
        enableCheck:SetPoint("RIGHT", tabContent, "RIGHT", -PADDING, 0)
        y = y - FORM_ROW
    ...
    local function BuildCharacterPaneTab(tabContent)
        ...
        local enableCheck = GUI:CreateFormCheckbox(tabContent, "PREY Character Module",
        ...
        local inspectEnabled = GUI:CreateFormCheckbox(tabContent, "Enable Inspect Overlays", "inspectEnabled", char, function()
            print("|cFFF87171PreyUI:|r Inspect overlay change requires /reload to take effect.")
        end)
    ...
    local function BuildDragonridingTab(tabContent)
        ...
        -- Set search context for auto-registration
        GUI:SetSearchContext({tabIndex = 2, tabName = "General & QoL", subTabIndex = 8, subTabName = "Dragonriding"})
...
local function CreateAutohidesPage(parent)
    ...
    local function BuildAutohideTab(tabContent)
        ...
        GUI:SetSearchContext({tabIndex = 6, tabName = "Autohide & Skinning", subTabIndex = 1, subTabName = "Autohide"})
    ...
    local function BuildSkinningTab(tabContent)
        ...
        GUI:SetSearchContext({tabIndex = 6, tabName = "Autohide & Skinning", subTabIndex = 2, subTabName = "Skinning"})
        ...
        if db and db.general then
            ...
            local function RefreshAllSkinning()
                ...
                if _G.PreyUI_RefreshAlertColors then
                    _G.PreyUI_RefreshAlertColors()
                end
                if _G.PreyUI_RefreshLootColors then
                    _G.PreyUI_RefreshLootColors()
                end
                if _G.PreyUI_RefreshMPlusTimerColors then
                    _G.PreyUI_RefreshMPlusTimerColors()
                end
                ...
                if _G.PreyUI_RefreshPowerBarAltColors then
                    _G.PreyUI_RefreshPowerBarAltColors()
                end
                if _G.PreyUI_RefreshGameMenuColors then
                    _G.PreyUI_RefreshGameMenuColors()
                end
                if _G.PreyUI_RefreshOverrideActionBarColors then
                    _G.PreyUI_RefreshOverrideActionBarColors()
                end
                if _G.PreyUI_RefreshObjectiveTrackerColors then
                    _G.PreyUI_RefreshObjectiveTrackerColors()
                end
                if _G.PreyUI_RefreshInstanceFramesColors then
                    _G.PreyUI_RefreshInstanceFramesColors()
                end
                if _G.PreyUI_RefreshReadyCheckColors then
                    _G.PreyUI_RefreshReadyCheckColors()
                end
            end
            ...
            if general.addPreyUIButton == nil then general.addPreyUIButton = false end
            ...
            local gameMenuCheck = GUI:CreateFormCheckbox(tabContent, "Skin Game Menu", "skinGameMenu", general, function()
                GUI:ShowConfirmation({
                    ...
                    acceptText = "Reload",
                    cancelText = "Later",
                    onAccept = function() PreyUI:SafeReload() end,
                })
            end)
            ...
            local addPREYButtonCheck = GUI:CreateFormCheckbox(tabContent, "Add PreyUI Button", "addPreyUIButton", general, function()
                GUI:ShowConfirmation({
                    title = "Reload UI?",
            ...
            addPREYButtonCheck:SetPoint("TOPLEFT", PAD, y)
            addPREYButtonCheck:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            y = y - FORM_ROW

            local gameMenuFontSlider = GUI:CreateFormSlider(tabContent, "Button Font Size", 8, 18, 1, "gameMenuFontSize", general, function()
                if _G.PreyUI_RefreshGameMenuFontSize then
                    _G.PreyUI_RefreshGameMenuFontSize()
                end
            end)
            ...
            local readyCheckDesc = GUI:CreateLabel(tabContent, "Skin the ready check popup with PREY styling.", 11, C.textMuted)
            ...
            local rcResetBtn = GUI:CreateButton(tabContent, "Reset Position", 140, 28, function()
                if _G.PreyUI_ResetReadyCheckPosition then
                    _G.PreyUI_ResetReadyCheckPosition()
                    print("|cFFF87171[PREY]|r Ready Check position reset to default.")
                end
            end)
            ...
            local desc = GUI:CreateLabel(tabContent, "Skin the M+ keystone insertion window with PREY styling.", 11, C.textMuted)
            ...
            local powerBarMoverBtn = GUI:CreateButton(tabContent, "Toggle Position Mover", 160, 28, function()
                if _G.PreyUI_TogglePowerBarAltMover then
                    _G.PreyUI_TogglePowerBarAltMover()
                end
            end)
            ...
            local lootDesc = GUI:CreateLabel(tabContent, "Replace Blizzard's loot window with a custom PREY-styled frame.", 11, C.textMuted)
            ...
            local rollDesc = GUI:CreateLabel(tabContent, "Replace Blizzard's loot roll frames with custom PREY-styled frames.", 11, C.textMuted)
            ...
            local function RefreshRollPreview()
                ...
                if PREYCore and PREYCore.Loot and PREYCore.Loot:IsRollPreviewActive() then
                    PREYCore.Loot:HideRollPreview()
                    PREYCore.Loot:ShowRollPreview()
                end
            end
            ...
            local historyDesc = GUI:CreateLabel(tabContent, "Apply PREY styling to the loot roll results panel.", 11, C.textMuted)
            ...
            local preyMplusHeader = GUI:CreateSectionHeader(tabContent, "PREY M+ Timer")
            preyMplusHeader:SetPoint("TOPLEFT", PAD, y)
            y = y - preyMplusHeader.gap

            local preyMplusDesc = GUI:CreateLabel(tabContent, "Custom M+ timer with PREY styling. Replaces the Blizzard timer with a clean, compact frame.", 11, C.textMuted)
            preyMplusDesc:SetPoint("TOPLEFT", PAD, y)
            preyMplusDesc:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            preyMplusDesc:SetJustifyH("LEFT")
            preyMplusDesc:SetWordWrap(true)
            preyMplusDesc:SetHeight(20)
            y = y - 24

            local preyMplusNote = GUI:CreateLabel(tabContent, "Disabled by default — most M+ players prefer dedicated timer addons. Enable for an all-in-one solution.", 10, {1.0, 0.75, 0.2, 1})
            preyMplusNote:SetPoint("TOPLEFT", PAD, y)
            preyMplusNote:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            preyMplusNote:SetJustifyH("LEFT")
            preyMplusNote:SetWordWrap(true)
            preyMplusNote:SetHeight(20)
            y = y - 28

            local preyMplusCheck = GUI:CreateFormCheckbox(tabContent, "Enable PREY M+ Timer", "enabled", mplusTimer, function()
                GUI:ShowConfirmation({
                    title = "Reload UI?",
            ...
            preyMplusCheck:SetPoint("TOPLEFT", PAD, y)
            ...
            preyMplusTimerCheck:SetPoint("TOPLEFT", PAD, y)
            ...
            preyMplusBorderCheck:SetPoint("TOPLEFT", PAD, y)
            preyMplusBorderCheck:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            y = y - FORM_ROW

            local preyMplusDeathsCheck = GUI:CreateFormCheckbox(tabContent, "Show Deaths", "showDeaths", mplusTimer, function()
                local MPlusTimer = _G.PreyUI_MPlusTimer
                if MPlusTimer and MPlusTimer.UpdateLayout then
                    MPlusTimer:UpdateLayout()
                end
            end)
            preyMplusDeathsCheck:SetPoint("TOPLEFT", PAD, y)
            preyMplusDeathsCheck:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            y = y - FORM_ROW

            local preyMplusAffixCheck = GUI:CreateFormCheckbox(tabContent, "Show Affixes", "showAffixes", mplusTimer, function()
                local MPlusTimer = _G.PreyUI_MPlusTimer
                if MPlusTimer and MPlusTimer.UpdateLayout then
                    MPlusTimer:UpdateLayout()
                end
            end)
            preyMplusAffixCheck:SetPoint("TOPLEFT", PAD, y)
            preyMplusAffixCheck:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            y = y - FORM_ROW

            local preyMplusObjCheck = GUI:CreateFormCheckbox(tabContent, "Show Objectives", "showObjectives", mplusTimer, function()
                local MPlusTimer = _G.PreyUI_MPlusTimer
                if MPlusTimer and MPlusTimer.UpdateLayout then
                    MPlusTimer:UpdateLayout()
                end
            end)
            preyMplusObjCheck:SetPoint("TOPLEFT", PAD, y)
            ...
            preyMplusDemoBtn:SetPoint("TOPLEFT", PAD, y)
            y = y - 40

            local preyMplusDemoInfo = GUI:CreateLabel(tabContent, "Demo mode shows a preview timer for testing.", 10, C.textMuted)
            preyMplusDemoInfo:SetPoint("TOPLEFT", PAD, y)
            preyMplusDemoInfo:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            ...
            local objTrackerDesc = GUI:CreateLabel(tabContent, "Apply PREY styling to quest objectives, achievement tracking, and bonus objectives.", 11, C.textMuted)
            ...
            local objTrackerHeightSlider = GUI:CreateFormSlider(tabContent, "Max Height", 200, 1000, 10,
                "objectiveTrackerHeight", general, function()
                    if _G.PreyUI_RefreshObjectiveTracker then _G.PreyUI_RefreshObjectiveTracker() end
                end)
...
local function CreateMinimapPage(parent)
    ...
    local function BuildMinimapTab(tabContent)
        ...
        if true then  -- Always build widgets (was: if db and db.minimap then)
            ...
            local enableCheck = GUI:CreateFormCheckbox(tabContent, "Enable PREY Minimap", "enabled", mm, RefreshMinimap)
            enableCheck:SetPoint("TOPLEFT", PAD, y)
            enableCheck:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
            y = y - FORM_ROW

            local lockCheck = GUI:CreateFormCheckbox(tabContent, "Lock PREY Minimap", "lock", mm, RefreshMinimap)
...
local function CreateCDMSetupPage(parent)
    ...
    local function BuildEssentialTab(tabContent)
        ...
        -- Set search context for auto-registration
        GUI:SetSearchContext({tabIndex = 7, tabName = "CDM Setup & Class Bars", subTabIndex = 1, subTabName = "Essential"})
    ...
    local function BuildUtilityTab(tabContent)
        ...
        -- Set search context for auto-registration
        GUI:SetSearchContext({tabIndex = 7, tabName = "CDM Setup & Class Bars", subTabIndex = 2, subTabName = "Utility"})
    ...
    local function BuildBuffTab(tabContent)
        ...
        GUI:SetSearchContext({tabIndex = 7, tabName = "CDM Setup & Class Bars", subTabIndex = 3, subTabName = "Buff"})
        ...
        if trackedData.texture == nil then trackedData.texture = "Prey v5" end
        if trackedData.useClassColor == nil then trackedData.useClassColor = true end
        if trackedData.barColor == nil then trackedData.barColor = {0.820, 0.180, 0.220, 1} end
    ...
    local function BuildPowerbarTab(tabContent)
        ...
        -- Set search context for widget auto-registration
        GUI:SetSearchContext({tabIndex = 7, tabName = "CDM Setup & Class Bars", subTabIndex = 4, subTabName = "Class Resource Bar"})
        ...
        -- New in this version: empty-slot overlay makes the bar visible at 0 resource
        if secondary.emptySlotColor == nil then secondary.emptySlotColor = { 0.25, 0.25, 0.25, 0.55 } end
        ...
        local function RefreshPowerBars()
            if _G.PreyUI and _G.PreyUI.PREYCore then
                ...
                if PREYCore.UpdatePowerBar then PREYCore:UpdatePowerBar() end
        ...
        snapPrimaryBtn:SetScript("OnClick", function()
            ...
            if essentialViewer and essentialViewer:IsShown() then
                ...
                if rawCenterX and rawCenterY and rawScreenX and rawScreenY then
                    ...
                    print("|cFFF87171PreyUI:|r Could not get screen positions. Try again.")
                end
            else
                print("|cFFF87171PreyUI:|r Essential Cooldowns viewer not found or not visible.")
            end
        end)
        ...
        snapUtilityBtn:SetScript("OnClick", function()
            ...
            if utilityViewer and utilityViewer:IsShown() then
                ...
                print("|cFFF87171PreyUI:|r Utility Cooldowns viewer not found or not visible.")
            end
        end)
        y = y - FORM_ROW
        ...
        snapSecEssentialBtn:SetScript("OnClick", function()
            if _G.PreyUI_RefreshNCDM then _G.PreyUI_RefreshNCDM() end
            local essentialViewer = _G.EssentialCooldownViewer
            if essentialViewer and essentialViewer:IsShown() then
                local rawCenterX, rawCenterY = essentialViewer:GetCenter()
                local rawScreenX, rawScreenY = UIParent:GetCenter()
        ...
        snapSecPrimaryBtn:SetScript("OnClick", function()
            local PREYCore = _G.PreyUI and _G.PreyUI.PREYCore
            local primaryBar = PREYCore and PREYCore.powerBar
            local primaryCfg = PREYCore and PREYCore.db and PREYCore.db.profile.powerBar
            if primaryBar and primaryBar:IsShown() and primaryCfg then
                ...
                print("|cFFF87171PreyUI:|r Primary Class Resource Bar not found or not visible. Enable it first.")
            end
        end)
        y = y - FORM_ROW
        ...
        local tickColorSecondary = GUI:CreateFormColorPicker(tabContent, "Tick Color (slot separators)", "tickColor", secondary, RefreshPowerBars)
        tickColorSecondary:SetPoint("TOPLEFT", PAD, y)
        tickColorSecondary:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
        y = y - FORM_ROW

        local emptySlotColorSecondary = GUI:CreateFormColorPicker(tabContent, "Empty Slot Color (visible at 0 resource)", "emptySlotColor", secondary, RefreshPowerBars)
        emptySlotColorSecondary:SetPoint("TOPLEFT", PAD, y)
        emptySlotColorSecondary:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
        y = y - FORM_ROW

        local emptySlotDesc = GUI:CreateLabel(tabContent, "Overlay color shown for unfilled slots (e.g. empty Holy Power orbs). Set alpha to 0 to disable.", 11, C.textMuted)
        emptySlotDesc:SetPoint("TOPLEFT", PAD, y)
        emptySlotDesc:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
        emptySlotDesc:SetJustifyH("LEFT")
        y = y - 20
        ...
        resetPowerColorsBtn:SetScript("OnClick", function()
            ...
            RefreshPowerBars()
            print("|cFFF87171PreyUI:|r Resource colors reset to defaults.")
        end)
        y = y - FORM_ROW
...
local function CreateCDEffectsPage(parent)
    ...
    -- Set search context for auto-registration
    GUI:SetSearchContext({tabIndex = 8, tabName = "CDM GCD & Effects"})
    ...
    local function RefreshEffects()
        if _G.PreyUI_RefreshCooldownEffects then _G.PreyUI_RefreshCooldownEffects() end
    end
    ...
    local swipeDesc = GUI:CreateLabel(content, "Control which animations appear on CDM icons. Prey's personal setup is to turn OFF all the below.", 11, C.textMuted)
    ...
    local function CreateAddEntrySection(parentFrame, barID, refreshCallback)
        ...
        dropZone:SetScript("OnReceiveDrag", function(self)
            ...
            if cursorType == "item" then
                local itemID = id1
                if itemID then
                    local trackerModule = PREY and PREY.PREYCore and PREY.PREYCore.CustomTrackers
    ...
    local function BuildTrackerBarTab(tabContent, barConfig, barIndex, subTabsRef)
        ...
        local function RefreshEntryList()
            ...
            for j, entry in ipairs(entries) do
                ...
                removeBtn:SetScript("OnClick", function()
                    if PREYCore and PREYCore.CustomTrackers then
                        PREYCore.CustomTrackers:RemoveEntry(barConfig.id, entry.type, entry.id, nil)
                    end
                    RefreshEntryList()
                end)
        ...
        local function LockToPlayer(corner)
            if barConfig.lockedToPlayer and barConfig.lockPosition == corner then
                ...
                if not playerFrame then
                    print("|cffff6666[PREY]|r Player frame not found")
                    return
                end
        ...
        local function LockToTarget(corner)
            if barConfig.lockedToTarget and barConfig.targetLockPosition == corner then
                ...
                if not targetFrame then
                    print("|cffff6666[PREY]|r Target frame not found")
                    return
                end
    ...
    table.insert(tabDefs, {
        ...
        builder = function(tabContent)
            ...
            local function RefreshScannedList()
                ...
                local scannerDB = PREY.db and PREY.db.global and PREY.db.global.spellScanner
                local listY = 0
                local rowHeight = 30
...
local function CreateUnitFramesPage(parent)
    ...
    local function BuildGeneralTab(tabContent)
        ...
        -- Set search context for auto-registration
        GUI:SetSearchContext({tabIndex = 3, tabName = "Single Frames & Castbars", subTabIndex = 1, subTabName = "General"})
        ...
        local editDesc = GUI:CreateLabel(tabContent, "Toggle Edit Mode to drag and reposition unit frames. Or use /prey editmode", 11, C.textMuted)
    ...
    local function BuildUnitTab(tabContent, unitKey)
        ...
        GUI:SetSearchContext({tabIndex = 3, tabName = "Single Frames & Castbars", subTabIndex = subTabInfo.index, subTabName = subTabInfo.name})
        ...
        local function RefreshAuras()
            ...
            if PREY_UF and PREY_UF.auraPreviewMode then
                if PREY_UF.auraPreviewMode[unitKey .. "_debuff"] then
                    _G.PreyUI_ShowAuraPreview(unitKey, "debuff")
                end
                if PREY_UF.auraPreviewMode[unitKey .. "_buff"] then
        ...
        previewTrack:SetScript("OnClick", function()
            ...
            UpdatePreviewToggle(isPreviewOn)
            if isPreviewOn then
                if _G.PreyUI_ShowUnitFramePreview then _G.PreyUI_ShowUnitFramePreview(unitKey) end
            else
                if _G.PreyUI_HideUnitFramePreview then _G.PreyUI_HideUnitFramePreview(unitKey) end
            end
        end)
        y = y - FORM_ROW
        ...
        if unitKey == "player" then
            widthCallback = function()
                RefreshUnit()
                if _G.PreyUI_UpdateLockedCastbarToFrame then
                    _G.PreyUI_UpdateLockedCastbarToFrame()
                end
            end
        end
           ...
           or unitKey == "pet" or unitKey == "targettarget" or unitKey == "boss" then
            ...
            debuffPreviewTrack:SetScript("OnClick", function()
                ...
                if isDebuffPreviewOn then
                    if _G.PreyUI_ShowAuraPreview then
                        _G.PreyUI_ShowAuraPreview(unitKey, "debuff")
                    end
                else
                    if _G.PreyUI_HideAuraPreview then
                        _G.PreyUI_HideAuraPreview(unitKey, "debuff")
                    end
                end
            end)
            y = y - FORM_ROW
            ...
            buffPreviewTrack:SetScript("OnClick", function()
                ...
                if isBuffPreviewOn then
                    ...
                    if _G.PreyUI_HideAuraPreview then
                        _G.PreyUI_HideAuraPreview(unitKey, "buff")
                    end
                end
            end)
            y = y - FORM_ROW
...
local function CreateActionBarsPage(parent)
    ...
    local function BuildMasterSettingsTab(tabContent)
        ...
        local enableCheck = GUI:CreateFormCheckbox(tabContent, "Enable PREY Action Bars",
        ...
        local tipText = GUI:CreateLabel(tabContent,
            "PREY hooks into Blizzard action bars to skin them. Position and resize bars via Edit Mode (Blizzard minimum padding: 2px). If you need actionbar paging (stance/form swapping), want to use action bars as your CDM, or prefer more control - disable PREY Action Bars and use a dedicated addon (e.g., Bartender4, Dominos).",
            11, C.warning)
        ...
        local function BuildBarSettingsIntoContainer(barKey, container, onOverrideChanged)
            ...
            if barKey == "bar1" then
                local pageArrowToggle = GUI:CreateFormCheckbox(container,
                    ...
                    function(val)
                        if _G.PreyUI_ApplyPageArrowVisibility then
                            _G.PreyUI_ApplyPageArrowVisibility(val)
                        end
                    end)
...
local function BuildImportExportTab(tabContent)
    ...
    GUI:SetSearchContext({tabIndex = 14, tabName = "PREY Import/Export", subTabIndex = 1, subTabName = "Import/Export"})

    local info = GUI:CreateLabel(tabContent, "Import and export PreyUI profiles", 11, C.textMuted)
    info:SetPoint("TOPLEFT", PAD, y)
    info:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
    info:SetJustifyH("LEFT")
    y = y - 28
    ...
    local function RefreshExportString()
        ...
        if PREYCore and PREYCore.ExportProfileToString then
            local str = PREYCore:ExportProfileToString()
            exportEditBox:SetText(str or "Error generating export string")
        else
            exportEditBox:SetText("PREYCore not available")
        end
    end
    RefreshExportString()
    ...
    local importBtn = GUI:CreateButton(tabContent, "IMPORT AND RELOAD", 200, 28, function()
        ...
        if not str or str == "" then
            print("|cffff0000PreyUI: No import string provided.|r")
            return
        end
        local PREYCore = _G.PreyUI and _G.PreyUI.PREYCore
        if PREYCore and PREYCore.ImportProfileFromString then
            local ok, err = PREYCore:ImportProfileFromString(str)
            if ok then
                print("|cffef4444PreyUI:|r Profile imported successfully!")
                print("|cffef4444PreyUI:|r Please type |cFFFFD700/reload|r to apply changes.")
            else
                print("|cffff0000PreyUI: Import failed: " .. (err or "Unknown error") .. "|r")
            end
        else
            print("|cffff0000PreyUI: PREYCore not available for import.|r")
        end
    end)
...
local function BuildPreyStringsTab(tabContent)
    ...
    GUI:SetSearchContext({tabIndex = 14, tabName = "PREY Import/Export", subTabIndex = 2, subTabName = "Prey's Strings"})

    local info = GUI:CreateLabel(tabContent, "Prey's personal import strings - select all and copy", 11, C.textMuted)
    info:SetPoint("TOPLEFT", PAD, y)
    info:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
    info:SetJustifyH("LEFT")
    y = y - 28
    ...
    if _G.PreyUI and _G.PreyUI.imports and _G.PreyUI.imports.EditMode then
        editModeString = _G.PreyUI.imports.EditMode.data or ""
    end
    ...
    local preyString = ""
    if _G.PreyUI and _G.PreyUI.imports and _G.PreyUI.imports.PREYProfile then
        preyString = _G.PreyUI.imports.PREYProfile.data or ""
    end

    local preyContainer = CreateScrollableTextBox(tabContent, BOX_HEIGHT, preyString)
    preyContainer:SetPoint("TOPLEFT", PAD, y)
    preyContainer:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
    table.insert(allTextBoxes, preyContainer.editBox)

    y = y - BOX_HEIGHT - 8

    local preyBtn = GUI:CreateButton(tabContent, "SELECT ALL", 120, 24, function()
        selectOnly(preyContainer.editBox)
    end)
    preyBtn:SetPoint("TOPLEFT", PAD, y)

    local preyTip = GUI:CreateLabel(tabContent, "then press Ctrl+C to copy", 11, C.textMuted)
    ...
    local preyDarkString = ""
    if _G.PreyUI and _G.PreyUI.imports and _G.PreyUI.imports.PREYProfileDarkMode then
        preyDarkString = _G.PreyUI.imports.PREYProfileDarkMode.data or ""
    end

    local preyDarkContainer = CreateScrollableTextBox(tabContent, BOX_HEIGHT, preyDarkString)
    preyDarkContainer:SetPoint("TOPLEFT", PAD, y)
    preyDarkContainer:SetPoint("RIGHT", tabContent, "RIGHT", -PAD, 0)
    table.insert(allTextBoxes, preyDarkContainer.editBox)

    y = y - BOX_HEIGHT - 8

    local preyDarkBtn = GUI:CreateButton(tabContent, "SELECT ALL", 120, 24, function()
        selectOnly(preyDarkContainer.editBox)
    end)
    preyDarkBtn:SetPoint("TOPLEFT", PAD, y)

    local preyDarkTip = GUI:CreateLabel(tabContent, "then press Ctrl+C to copy", 11, C.textMuted)
    ...
    local platString = ""
    if _G.PreyUI and _G.PreyUI.imports and _G.PreyUI.imports.Platynator then
        platString = _G.PreyUI.imports.Platynator.data or ""
    end
...
local function CreateImportExportPage(parent)
    ...
    local subTabs = GUI:CreateSubTabs(content, {
        ...
        {name = "Prey's Strings", builder = BuildPreyStringsTab},
    })
    subTabs:SetPoint("TOPLEFT", 5, -5)
    subTabs:SetPoint("TOPRIGHT", -5, -5)
...
local function CreateSpecProfilesPage(parent)
    ...
    resetBtn:SetScript("OnClick", function()
        if db then
            GUI:ShowConfirmation({
                ...
                onAccept = function()
                    local PREYCore = _G.PreyUI and _G.PreyUI.PREYCore
                    local dbRef = PREYCore and PREYCore.db
                    if dbRef then
                        dbRef:ResetProfile()
                        print("|cffef4444PreyUI:|r Profile reset to defaults.")
                        print("|cffef4444PreyUI:|r Please type |cFFFFD700/reload|r to apply changes.")
                    end
                end,
            })
        ...
        for i, profileName in ipairs(profiles) do
            ...
            item:SetScript("OnClick", function()
                if profileName ~= currentProfile then
                    ...
                    print("|cffef4444PreyUI:|r Switched to profile: " .. profileName)
                end
                profileMenu:Hide()
            end)
        end
    ...
    createBtn:SetScript("OnClick", function()
        ...
        if newName and newName ~= "" and db then
            ...
            print("|cffef4444PreyUI:|r Created new profile: " .. newName)
        end
    end)
    y = y - FORM_ROW - 10
    ...
    local copyDropdown = GUI:CreateFormDropdown(content, "Copy From", GetProfileList(), "selected", copyWrapper, function(value)
        if db and value and value ~= "" then
            ...
            print("|cffef4444PreyUI:|r Copied settings from: " .. value)
            ...
            if value == current then
                ...
                GUI:ShowConfirmation({
                    ...
                    onAccept = function()
                        ...
                        print("|cffef4444PreyUI:|r Deleted profile: " .. profileToDelete)
                        deleteWrapper.selected = ""
                    end,
                })
            end
    ...
    if db and db.IsDualSpecEnabled and db.SetDualSpecEnabled and db.GetDualSpecProfile and db.SetDualSpecProfile then
        ...
        local enableCheckbox = GUI:CreateFormCheckbox(content, "Enable Spec Profiles", "enabled", enableWrapper,
            function()
                ...
                print("|cffef4444PreyUI:|r Spec auto-switch " .. (enableWrapper.enabled and "enabled" or "disabled"))
            end)
            ...
            if specName then
                ...
                local specDropdown = GUI:CreateFormDropdown(content, displayName, GetProfileList(), "selected", specWrapper, function(value)
                    if value and value ~= "" then
                        ...
                        print("|cffef4444PreyUI:|r " .. specName .. " will use profile: " .. value)
                    end
                end)
...
local function CreateHUDLayeringPage(parent)
    ...
    local function RefreshPowerBars()
        if PREYCore and PREYCore.UpdatePowerBar then
            PREYCore:UpdatePowerBar()
        end
        if PREYCore and PREYCore.UpdateSecondaryPowerBar then
            PREYCore:UpdateSecondaryPowerBar()
        end
    end
    ...
    local function RefreshCastbars()
        if _G.PreyUI_RefreshCastbars then
            _G.PreyUI_RefreshCastbars()
        end
    end
...
local function CreateStaggerBarPage(parent)
    ...
    -- Set search context
    GUI:SetSearchContext({tabIndex = 12, tabName = "Stagger Bar", subTabIndex = 1, subTabName = "Stagger Bar"})
    ...
    resetButton:SetScript("OnClick", function()
        StaticPopupDialogs["PREYUI_RESET_STAGGER"] = {
            ...
            OnAccept = function()
                ...
                if addon and addon.StaggerDefaults then
                    ...
                    print("|cfffb7185PreyUI:|r Stagger Bar reset to defaults. Reopen options to see changes.")
                end
            end,
            timeout = 0,
            whileDead = true,
        ...
        StaticPopup_Show("PREYUI_RESET_STAGGER")
    end)
    y = y - 40
...
function GUI:InitializeOptions()
    ...
    -- Row 1: Theme first — most users start here
    GUI:AddTab(frame, "Appearance & Theme", CreateAppearancePage)
    ...
    GUI:AddActionButton(frame, "Toggle Cooldown Settings", function()
        if CooldownViewerSettings then
            ...
            print("|cFFF87171PreyUI:|r Cooldown Settings not available. Enable Cooldown Manager in Options > Gameplay Enhancement.")
        end
    end)
