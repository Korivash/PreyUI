...
local function GetDungeonInfo(mapID)
    ...
    if _G.PREY_DungeonData then
        local data = _G.PREY_DungeonData.GetDungeonData(mapID)
        if data then
...
local function GetKeyColor(level)
    ...
    if _G.PREY_DungeonData and _G.PREY_DungeonData.GetKeyColor then
        return _G.PREY_DungeonData.GetKeyColor(level)
    end
    -- Fallback
...
local KeyTrackerFrame = CreateFrame("Frame", "PREYKeyTrackerFrame", UIParent, "BackdropTemplate")
...
-- Apply skin colors (consolidated - called after PREY is loaded)
...
_G.PreyUI_RefreshKeyTrackerColors = ApplySkinColors
...
_G.PreyUI_RefreshKeyTrackerFonts = UpdateAllButtonFonts
...
KeyTrackerFrame:SetScript("OnMouseUp", function(self, button)
    if button == "RightButton" and not InCombatLockdown() then
        RequestKeystones()
        print("|cFFF87171PreyUI:|r Refreshing party keys...")
    end
end)
