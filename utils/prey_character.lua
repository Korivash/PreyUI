...
local C = {
    ...
    -- Status colors
    enchanted = { 0.820, 0.180, 0.220, 1 },      -- Green/Mint (enchanted)
...
---------------------------------------------------------------------------
-- Get global font from PREY settings
---------------------------------------------------------------------------
...
local function HideBlizzardDecorations()
    ...
    local function SkinEquipmentSlot(slot)
        ...
        -- Block Blizzard's IconBorder (we use custom _preyBorderFrame instead)
...
---------------------------------------------------------------------------
-- Create custom PreyUI background
---------------------------------------------------------------------------
...
ScheduleUpdate = function()
    ...
    C_Timer.After(0.05, function()
        ...
        -- Update inspect frame if visible (delegated to prey_inspect.lua)
...
local function HookCharacterFrame()
    ...
    if not CharacterFrame._preyGearBtn then
        ...
        if generalDB then
            ...
            -- Refresh color picker when panel shows (in case color changed in main PREY options)
...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" then
        -- Note: Blizzard_InspectUI is now hooked by prey_inspect.lua
...
---------------------------------------------------------------------------
-- Shared exports for prey_inspect.lua
