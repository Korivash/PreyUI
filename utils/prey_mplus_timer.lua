...
local function GetPosition()
    ...
    local PREYCore = _G.PreyUI and _G.PreyUI.PREYCore
    if PREYCore and PREYCore.db and PREYCore.db.profile and PREYCore.db.profile.mplusTimer then
        local pos = PREYCore.db.profile.mplusTimer.position
        if pos then
            return {
...
local function SavePosition(point, relPoint, x, y)
    ...
    if PREYCore and PREYCore.db and PREYCore.db.profile then
        if not PREYCore.db.profile.mplusTimer then
            PREYCore.db.profile.mplusTimer = {}
        end
        PREYCore.db.profile.mplusTimer.position = {
            point = point,
            relPoint = relPoint,
            x = x,
            y = y
...
function MPlusTimer:CreateFrames()
    ...
    -- Root frame
    local root = CreateFrame("Frame", "PREY_MPlusTimerFrame", UIParent)
...
function MPlusTimer:EnableDemoMode()
    if self.state.inChallenge then
        print("|cFF34D4E8[PREY M+ Timer]|r Can't enable demo mode during active M+!")
        return
    end
    ...
    print("|cFF34D4E8[PREY M+ Timer]|r Demo mode enabled. Type /preympt demo to disable.")
end
...
function MPlusTimer:DisableDemoMode()
    ...
    print("|cFF34D4E8[PREY M+ Timer]|r Demo mode disabled.")
end
...
-- Slash Command
---------------------------------------------------------------------------
SLASH_PREYIMPLUSTIMER1 = "/preympt"
SLASH_PREYIMPLUSTIMER2 = "/qmpt"
SlashCmdList["PREYIMPLUSTIMER"] = function(msg)
    ...
    elseif cmd == "hide" then
        ...
        print("|cFF34D4E8[PREY M+ Timer]|r Commands:")
        print("  /preympt demo - Toggle demo mode")
        print("  /preympt show - Show timer")
        print("  /preympt hide - Hide timer")
    end
end
...
-- Expose for skinning
---------------------------------------------------------------------------
_G.PreyUI_MPlusTimer = MPlusTimer
