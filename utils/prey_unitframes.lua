...
PREY_UF.castbars = {}
PREY_UF.previewMode = {}
PREY_UF.auraPreviewMode = {}  -- Tracks buff/debuff preview state (keyed by "unitKey_buff" or "unitKey_debuff")
...
---------------------------------------------------------------------------
-- HELPER: Pixel-perfect scaling (uses PREYCore:Scale if available)
---------------------------------------------------------------------------
...
local function GetHealthBarColor(unit, settings)
    ...
    -- Get global settings from MAIN profile (not legacy quiUnitFrames sub-table)
    local general = GetGeneralSettings()
...
local function CreateBossFrame(unit, frameKey, bossIndex)
    ...
    -- Create secure button with unique name (PREY_Boss1, PREY_Boss2, etc.)
    ...
    if not self.exitEditModeBtn then
        ...
        exitBtn:SetScript("OnClick", function()
            if EditModeManagerFrame and EditModeManagerFrame:IsShown() then
                ...
                -- Fallback for /prey editmode case
...
function PREY_UF:HideBlizzardFrames()
    ...
    -- Hide Blizzard Player Castbar if our PREY player castbar is enabled
    ...
    -- Hide Focus frame visuals (always hide Blizzard focus frame when PREY unit frames are enabled)
...
---------------------------------------------------------------------------
-- Hook Blizzard's Edit Mode to trigger PREY's edit mode when user enters via Esc > Edit Mode
function PREY_UF:HookBlizzardEditMode()
    if not EditModeManagerFrame then return end
    if self._blizzEditModeHooked then return end
    self._blizzEditModeHooked = true

    -- Track if we triggered from Blizzard Edit Mode (vs /prey editmode)
    self.triggeredByBlizzEditMode = false

    hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
        if InCombatLockdown() then return end
        if self.editModeActive then return end  -- Already active via /prey editmode
    ...
    hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
        ...
        if not self.triggeredByBlizzEditMode then return end  -- Don't exit if user used /prey editmode
        self.triggeredByBlizzEditMode = false
        self:DisableEditMode()
    end)
end

-- Hide Blizzard's selection frames when PREY frames are enabled
...
function PREY_UF:HideBlizzardSelectionFrames()
    local function HideSelection(parent, unitKey)
        ...
        -- Hook OnShow to persistently hide while PREY frames are enabled
...
-- Castbar functions are now in prey_castbar.lua module
