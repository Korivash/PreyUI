...
local function CreatePowerBarNudgeButton(parent, direction, deltaX, deltaY, barKey)
    ...
    btn:SetScript("OnClick", function()
        local cfg = (barKey == "primary") and PREYCore.db.profile.powerBar or PREYCore.db.profile.secondaryPowerBar
            ...
            bar:SetScript("OnMouseDown", function(self, button)
                if button == "LeftButton" and PowerBarEditMode.active then
                    if PREYCore and PREYCore.SelectEditModeElement then
                        PREYCore:SelectEditModeElement("powerbar", self._editModeBarKey)
                    end
                end
            end)
...
function PREYCore:UpdatePowerBar()
    local cfg = self.db.profile.powerBar
...
_G.PreyUI_UpdateLockedPowerBar = function()
    ...
    local cfg = PREYCore.db.profile.powerBar
...
_G.PreyUI_UpdateLockedSecondaryPowerBar = function()
    ...
    local cfg = PREYCore.db.profile.secondaryPowerBar
    if not cfg.enabled or not cfg.lockedToEssential then return end
...
function PREYCore:CreateFragmentedPowerBars(bar, resource, isVertical)
    local cfg = self.db.profile.secondaryPowerBar
...
function PREYCore:UpdateFragmentedPowerDisplay(bar, resource, isVertical)
    local cfg = self.db.profile.secondaryPowerBar
    local maxPower = UnitPowerMax("player", resource)
...
function PREYCore:UpdateSecondaryPowerBar()
    local cfg = self.db.profile.secondaryPowerBar
    if not cfg.enabled then
    ...
    C_Timer.After(0.6, function()
        if EditModeManagerFrame and not PREYCore._powerBarEditModeHooked then
            ...
            hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
                if not InCombatLockdown() then
                    PREYCore:EnablePowerBarEditMode()
                end
            end)
            hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
                if not InCombatLockdown() then
                    PREYCore:DisablePowerBarEditMode()
                end
            end)
        end
    end)
