...
local function HookExtraButtonPositioning()
    ...
    -- Only do this if PREY is actually managing these frames (at least one feature enabled)
    local extraSettings = GetExtraButtonDB("extraActionButton")
    local zoneSettings = GetExtraButtonDB("zoneAbility")
    local preyManagingFrames = (extraSettings and extraSettings.enabled) or (zoneSettings and zoneSettings.enabled)
    -- BUG-008: Use C_Timer.After(0) and combat check to avoid taint propagation to secure frame manager
    if preyManagingFrames and UIParentBottomManagedFrameContainer and ExtraAbilityContainer then
